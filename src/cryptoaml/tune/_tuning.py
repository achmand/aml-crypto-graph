...
class OptunaTuner(_BaseTuner):
    ...
    def fit(self, X, y):
        ...
        if is_boosting:
            #TODO -> Remove n_jobs
            self._study.optimize(self._objective, n_trials=self._n_iterations, n_jobs=4) 
        else:
            #TODO -> Remove n_jobs
            self._study.optimize(self._objective_normal, n_trials=self._n_iterations, n_jobs=4) 
